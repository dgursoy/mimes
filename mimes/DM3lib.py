...
Doga Gursoy <dgursoy@aps.anl.gov

This Python module is a transposition and adaptation of the DM3_Reader 
ImageJ plug-in (http://rsb.info.nih.gov/ij/plugins/DM3_Reader.html) 
by Greg Jefferis <jefferis@stanford.edu>.

It allows to extract thumbnail, image data and metadata (specimen, 
operator, HV, MAG, etc.) from GATAN DigitalMicrograph 3 files (it 
was initially meant to be called by a script indexing electron 
microscope images in a database). In particular, it can dump all 
metadata (“Tags”), pass thumbnail and image data as PIL Images or 
Numpy arrays, as well as save the thumbnail view in a PNG file.
...
# Thumbnail to Array
def thumb2ar( image_ ):
    """Convert PIL Thumbnail to Numpy array."""
    if image_.mode in ('L', 'I', 'F'):
        # Warning: only works with PIL.Image.Image whose mode is 'L', 'I' or 'F'
        #          => error if mode == 'I;16' for instance
        return fromimage( image_ )
...
class DM3(object):
    ...
    def __init__(self, filename, debug=0, dims=2):
    ...
    def thumbnaildata(self):
        ...
        return thumb2ar(self.thumbnail)
    ...
    def image(self):
        ...
        if self.debug > 0:
            ...
            if self.dims == 2:
                ...
                print "Notice: image size: %sx%sx%s px" % (im_depth, im_width, im_height)
    ...
    def imagedata(self):
        ...
        if self.dims == 2:
            ...
            try:
                ...
                im_depth = 1
        return im2ar(self.image, im_depth)
    ...
    def saveh5(self, fname):
        """Converts DM3 to HDF5 file"""
        import h5py
        f = h5py.File(fname, 'w')
        for entry in self.tags:
            f.create_dataset(entry.replace(".","/"), data=self.tags[entry])
        exchangeGrp = f.create_group("exchange")
        exchangeGrp.create_dataset("data", data=self.imagedata)
        f.close()
        if (self.debug > 3):
            print "Notice: Data saved as H5"
